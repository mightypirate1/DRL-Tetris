...
class prio_qnet:
    ...
    def train(self, vector_states, visual_states, actions, state_pieces, rewards, dones, weights=None, lr=None):
        run_list = [
                    self.training_ops,
                    self.new_prios_tf,
                    self.stats_tensors,
    ...
    def create_q_head(self,vectors, visuals, name):
        with tf.variable_scope("q-net-"+name, reuse=tf.AUTO_REUSE) as vs:
            ...
            q = tf.reshape(_q, [-1, *self.output_shape], name='q-values',)
        return q, scope
    ...
    def create_duelling_qnet(self, vector_states, visual_states, vector_states_training, visual_states_training, actions, pieces, actions_training, pieces_training, rewards, dones):
        with tf.variable_scope("prio_q-net") as vs:
            q_tf, main_scope = self.create_q_head(vector_states, visual_states, "main")
            ...
            # # #
            # Trainers do {1,...,k}-step Q-estimators in steps:
            # # #

            #1) get all the values and a bool to tell if the round is over
            q_i_tf = [0 for _ in range(self.k_step+1)]
            ...
            for i in range(self.k_step+1):
                ...
                q_i_tf[i], ref_scope = self.create_q_head(subinputs_vec, subinputs_vis, subscope)
            ...
            def k_step_estimate(k):
                ...
                expected_q = tf.reduce_sum(q_i_tf[k] * self.static_mask, axis=2) / tf.reduce_sum(self.static_mask, axis=2) #Q-values averaged over all possible next-pieces
                val = tf.reduce_max(expected_q, axis=1, keepdims=True)
                e += val * tf.cast((done_time_tf > k),tf.float32) * (gamma**k)
                return e
            ...
            a_mask = tf.reshape( tf.one_hot(actions_training[:,0,:], self.n_actions), [-1, self.n_actions, 1] )
            p_mask = tf.reshape( tf.one_hot(pieces_training[:,0,:], self.n_pieces),   [-1, 1, self.n_pieces ] )
            training_values_tf = tf.expand_dims(tf.reduce_sum( (q_i_tf[0] * a_mask) * p_mask, axis=(1,2)),1)
