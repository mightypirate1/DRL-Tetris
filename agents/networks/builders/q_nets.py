...
class q_net_silver(q_net_base):
    def Q_V_A(self, vec, vis, one_eyed_advantage=False):
        ...
        if not self.worker_only:
            ...
            _V = self.settings["piece_advantage_range"] * N.normalize_advantages(_V, axis=1, piece_mask=self.used_pieces_mask_tf, n_used_pieces=self.n_used_pieces) #This works here too!
        else:
            _V = tf.constant([0.0], dtype=tf.float32)
        V_qshaped = tf.reshape(_V,[-1,1,1,_V.shape.as_list()[-1]]) #Shape for Q-calc!

        #Finnishing touch
        A = self.settings["advantage_range"] * N.normalize_advantages(_A, separate_piece_values=self.settings["separate_piece_values"], mode=self.settings["advantage_type"], piece_mask=self.used_pieces_mask_tf, n_used_pieces=self.n_used_pieces)
        Q = V_qshaped + A
