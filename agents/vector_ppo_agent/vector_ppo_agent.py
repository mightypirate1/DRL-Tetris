...
from agents.vector_ppo_agent.vector_ppo_agent_base import vector_ppo_agent_base
from agents.vector_ppo_agent.vector_ppo_agent_trainer import vector_ppo_agent_trainer
...
class vector_ppo_agent(vector_ppo_agent_base):
    def __init__(
                 self,
        ...
        vector_ppo_agent_base.__init__(self, id=id, name="worker{}".format(id), session=session, sandbox=sandbox, settings=settings, mode=mode)
        #Helper variables
    ...
    def get_action(self, state_vec, player=None, random_action=False, training=False, verbose=False):
        ...
        for i, (state, _piece, player) in enumerate(zip(state_vec,pieces,p_list)):
            ...
            if distribution == "argmax": #for eval-runs
                ...
                (r, t), entropy = q.action_distribution(PI[i,:,:,piece])
